...
void WeightsSerializer::_validateFile(const std::string& path)
{
	...
	std::streampos filePos = fileStream.seekg(0, std::ios_base::beg).tellg();
	...
	for(size_t blockIdx = 0; blockIdx < nBlocks; blockIdx++)
	{
		...
		if((fileEnd - filePos) < 8 * nDimensions)
		{
			LOG_ERROR("Layers::WeightsSerializer",
		...
		if((fileEnd - filePos) < 8 * tensorSize)
		{
			LOG_ERROR("Layers::WeightsSerializer",
...
void WeightsSerializer::_initHandles()
{
	...
	{
		const auto& newestHandle = _tensorHandles.emplace_back(*_fileStream, currentPos);
...
mlCore::Tensor TensorHandle::get() const
{
	...
	_file.seekg(_position + 8 + 8 * shape.size());
