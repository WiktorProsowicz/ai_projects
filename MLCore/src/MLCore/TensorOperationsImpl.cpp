 ...
 * @param shape2 
 */
void checkShapesForBroadcasting(const std::vector<size_t>& shape1,
		...
		if((*leftShapeIter != 1) && (*rightShapeIter != 1) && (*leftShapeIter != *rightShapeIter))
		{
			LOG_ERROR("TensorOperations",
					  "Can't perform broadcasting operation on tensors with invalid shapes: "
						  << stringifyVector(shape1) << " " + stringifyVector(shape2));
		}
	}
}
 ...
 */
std::vector<size_t> padShapeFromLeft(const std::vector<size_t>& shape,
									 const size_t biggerSize,
									 const size_t paddingVal = 1)
{
										   ...
										   const std::vector<size_t>& paddedShape2)
{
	...
	for(size_t shapePos = 0; shapePos < paddedShape1.size(); shapePos++)
	{
		...
		retShape[shapePos] =
			paddedShape1[shapePos] == 1 ? paddedShape2[shapePos] : paddedShape1[shapePos];
	}
